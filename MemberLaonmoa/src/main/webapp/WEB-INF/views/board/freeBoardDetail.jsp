...
<body id="fdsafdsa">
			...
			<div class="in">
				<table class="table">
					<thead>
						...
						<tr>
							<td style="text-align: right" colspan="3"><fmt:formatDate
									var="date" value="${detail.date}" pattern="yyyy-MM-dd HH:mm" />
								작성일자 : ${date}</td>
						</tr>
					</thead>
					<tbody>
						<tr style="height: 500px">
							<td colspan="3" style="text-align: left">${detail.content}<br>
							</td>
						</tr>
						<tr>
							...
							<td colspan="3" align="right"><c:if test="${id ne null}">
								...
								href="${ContextPatn}/board/freeBoardUpdate?bnum=<%=bnum %>&pagenum=<%=pagenum %>">
						...
						<tr>
							<td colspan="3"><textarea id="rcontent"
								...
								<button class="btn laonmoabtn" style="margin-right: 15px"
									onclick="replysend()">댓글 입력</button></td>
						</tr>
	...
	function updatecencel(idx){
		$("#hidden"+idx).hide();
		$("#show"+idx).show();
		$("#rcontentupdate"+idx).val($("#replycontent"+idx).text());
	}
	...
	function replyregister(){
		...
		$.ajax({
			type:"get",
			url:"${ContextPath}/board/freeReplyInsert",
			data:query,
			success:function(data){
				if(data == 1){
					replyList();
	...
	function replyupdate(idx,index){
		...
		$.ajax({
			type:"post",
			url:"${ContextPath}/board/freeReplyUpdate",
			data:query,
			success:function(data){
				if(data == 1){
					replyList();
	...
	function replydelete(idx){
		if(confirm("정말 삭제하시겠습니까?")==true){
			...
			$.ajax({
				type:"post",
				url:"${ContextPath}/board/freeReplyDelete",
				data:{rnum:rnum},
				success:function(data){
					replyList();
				},
	...
	function back(){
		...
		var url1 = referrer.match("freeBoardSearch");
		if(url == "myReview" || url1 == "freeBoardSearch" || cut.length < 5){
			history.back();
		}
		else{
		...
		if(confirm("정말 삭제하시겠습니까?")==true){
			var query={
				...
				$.ajax({
					...
					data:query,
					success:function(data){
						location.replace("${ContextPath}/board/freeBoardMain?pagenum=1");
					},
					error:function(data){
						alert("실패"); 
					}
