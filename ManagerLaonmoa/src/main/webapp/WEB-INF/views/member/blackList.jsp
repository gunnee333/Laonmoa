...
<div class="main_menu_in"><%@ include file="../include/adminMenu.jsp"%></div>
	<div class="sub"><div class="out"><div class="in">
			...
			<tbody>
				...
				<c:forEach var="list" items="${list}">
					<tr align="center" class="td_style">
						<td><a style="color: blue" href="memberView?id=${list.id}&pagenum=<%=pagenum%>&level=2">${list.id}</a>
						</td>
						<td>${list.name }</td>
						<td>${list.email }</td>
						<td>${list.tel }</td>
		...
		<!-- 검색내용 입력 -->
		<form id="form" method="get" action="${ContextPath }/admin/blacksearch">
...
<script>
	...
	function page(idx){
		...
		if(!(keyword == "" || keyword == null || keyword =="null")){
			location.replace("${ContextPath }/admin/blacksearch?pagenum="
					+pagenum+"&level=2&keyword="+keyword+"&searchmember="+searchmember);
		}
		else{
	...
	function max(idx){
		...
		if(!(keyword == "" || keyword == null || keyword == "null")){
			location.replace("${ContextPath }/admin/blacksearch?pagenum="
			+pagenum+"&level=2&keyword="+keyword+"&searchmember="+searchmember+"&max="+idx);
		}
		else{
			location.replace("${ContextPath}/admin/blackList?pagenum="+pagenum+"&max="+idx+
					"&level=2");
		}
	}
</script>
